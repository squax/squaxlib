...
{
    [CustomPropertyDrawer(typeof(SelectInActionAttribute))]
    public class SelectInActionPropertyDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (selectedIndex > 0)
            {
                property.objectReferenceValue = action.In[selectedIndex - 1];
            }
            ...
            if (Action.CopyAction != null && action.In.Contains(Action.CopyAction) && GUI.Button(position, "Paste Node"))
            {
